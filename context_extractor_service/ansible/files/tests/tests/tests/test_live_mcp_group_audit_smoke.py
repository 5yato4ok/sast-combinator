...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in flow-aware audit on cloud_portal authorization.ts: "
        "_candidate_function_name extracts the return type 'void' from an anonymous "
        "TypeScript arrow wrapper and sends the next flow step into irrelevant route lookup."
    ),
)
