  ...
  try {
    ...
    switch (args[0]) {
      ...
      case '-e':
        ...
        } else if (gitDataInput) {
          gitData = JSON.parse(gitDataInput);
        } else {
          throw new Error('Git data required for extraction');
        }
      ...
      case '--issues':
      case '-i':
        const issueDataInput = args[1];
        let issueData;

        if (issueDataInput && issueDataInput.endsWith('.json')) {
          issueData = JSON.parse(fs.readFileSync(issueDataInput, 'utf8'));
        } else if (issueDataInput) {
          issueData = JSON.parse(issueDataInput);
        } else {
          throw new Error('Git data required for issue extraction');
        }

        result = extractor.extractIssues(issueData);
        break;
      ...
      case '-s':
        ...
        if (commitsInput && commitsInput.endsWith('.json')) {
          ...
          throw new Error('Commits data required for semantic analysis');
        }
      ...
      case '-c':
        ...
        if (filesInput && filesInput.endsWith('.json')) {
          ...
          throw new Error('Files data required for component detection');
        }
      ...
      case '-m':
        ...
        if (metaDataInput && metaDataInput.endsWith('.json')) {
          ...
          throw new Error('Git data required for metadata extraction');
        }
