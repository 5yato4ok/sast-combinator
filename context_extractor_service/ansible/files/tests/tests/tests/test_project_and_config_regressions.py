...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in live MCP audit on nx-connect-ui: Jenkinsfile is routed as production "
        "instead of a config/build artifact, which sends the agent down the wrong tool flow."
    ),
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in live cloud_portal findings: deployment/build helper scripts are routed as "
        "production instead of config/tooling artifacts, which sends the agent down the wrong flow."
    ),
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in live cloud_portal findings: auxiliary tooling scripts are routed as "
        "production instead of config/tooling artifacts."
    ),
)
@pytest.mark.parametrize(
    "path",
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in config-flow validation: docker-compose.override.yml is classified as "
        "production instead of a config/dev artifact, so the agent enters the wrong flow."
    ),
)
