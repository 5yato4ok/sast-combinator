...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed locally while reproducing cloud_portal-like TypeScript service flows: "
        "find_callers returns the method definition file before the actual invocation site."
    ),
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in flow-aware live audit on nx: find_callers returns C++ declaration and "
        "definition sites before the real invocation site."
    ),
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "Observed in live generate-customization.js flow: main().catch(error => ...) "
        "is reported with caller_function='error' instead of top-level/no enclosing caller."
    ),
)
