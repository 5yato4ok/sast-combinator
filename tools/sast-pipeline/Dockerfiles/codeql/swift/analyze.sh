...
OUTPUT_FILE="${OUTPUT_DIR}/${3:-codeql_swift.sarif}"

mkdir -p "$OUTPUT_DIR"
DB_DIR="/tmp/codeql-db-swift"
rm -rf "$DB_DIR" && mkdir -p "$DB_DIR"

echo "[INFO] Creating CodeQL DB for swift"
codeql database create "$DB_DIR" --language="swift" --source-root "$INPUT_DIR"

QPKG="codeql/swift-queries"
